...
function particle_make_more() {
  ...
  // Get the height of the visible viewport, adjusting for mobile behavior
  const viewportHeight = document.documentElement.clientHeight;

  // Calculate the percentage of the page height relative to the viewport height
  const pageHeightPercentage = (pageHeight / viewportHeight) * 100;

  setTimeout(particle_make_more, random(0, 500));
  let l = random(1, 25);
  for (let i = 0; i < l; i++) {
    particle_make(random(pageHeightPercentage, pageHeightPercentage - 3));
  }
}
